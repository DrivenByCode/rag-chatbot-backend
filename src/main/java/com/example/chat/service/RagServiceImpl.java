...
public class RagServiceImpl implements RagService {
    ...
    public ChatResponse ask(String sessionId, String question) {
        ...
        if (!chatbotProperties.isJustLlm()) {
            ragContexts = fetchRagContexts(question);

            if (chatbotProperties.isRagOnly() && ragContexts.isEmpty()) {
            ...
            if (chatbotProperties.isRagOnly()) {
                String reply = RandomUtil.getRandomElement(ragContexts);
                memoryService.saveMessage(sessionId, new ChatMessage("bot", reply));
                return ChatResponse.withReferences(reply, "stop", ragContexts);
            }
        }
        ...
        if (!ragContexts.isEmpty()) {
            return ChatResponse.withReferences(reply, finishReason, ragContexts);
        } else {
    ...
    /**
     * 질문에 관련된 RAG 컨텍스트를 가져옵니다.
     */
    private List<String> fetchRagContexts(String question) {
        try {
    ...
    private Prompt buildPrompt(String sessionId, String question, String summary, List<String> ragContexts) {
        ...
        if (!chatbotProperties.isJustLlm()) {
            ...
            List<ChatMessage> recent = memoryService.getRecentMessages(sessionId, 8);
